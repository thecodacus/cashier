...
const LinkItems: Array<LinkItemProps> = [
	...
	{ name: "Products", icon: MdOutlineInventory2, url: "/products" },
	{ name: "Checkouts", icon: MdProductionQuantityLimits, url: "/checkouts" },
...
export default function SidebarWithHeader({ children }: { children: ReactNode }) {
	...
	return (
		<Box minH="100vh" height="100vh" bg={useColorModeValue("gray.100", "gray.900")}>
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
	return (
		<Box transition="200ms ease" bg={useColorModeValue("white", "gray.900")} borderRight="1px" borderRightColor={useColorModeValue("gray.200", "gray.700")} w={{ base: "full", md: 60 }} pos="fixed" h="full" {...rest}>
			...
			{LinkItems.map((link) => (
				<NavItem key={link.name} as={"div"} icon={link.icon} url={link.url}>
					{link.name}
				</NavItem>
			))}
		</Box>
...
const MobileNav = ({ onOpen, ...rest }: MobileProps) => {
	const user = useAppSelector((s) => s.auth.claims);
	return (
		<Flex
			ml={{ base: 0, md: 60 }}
			px={{ base: 4, md: 4 }}
